...
def part2_inverse_kinematics(meta_data, joint_positions, joint_orientations, relative_x, relative_z, target_height):
    ...
    for i in range(len(meta_data.joint_name)):
        parent_idx = meta_data.joint_parent[i]
        if i  in path :
            rotation_local = R.from_euler('XYZ', [0, 0,0], degrees=True)
            parent_rot = R.from_quat(ik_joint_orientations[parent_idx])
            angles_T = parent_rot * rotation_local
            ik_joint_orientations[i] = angles_T.as_quat()
            ik_joint_positions[i] = ik_joint_positions[parent_idx] + angles_T.apply(joint_offsets[i])
...
def bonus_inverse_kinematics(meta_data, joint_positions, joint_orientations, left_target_pose, right_target_pose):
    ...
    for iter in range(max_iterations):
        ...
        # 2. 计算误差
        end_effector_pos = ik_joint_positions[path_to_end_effector[-1]]
        error =  end_effector_pos - left_target_pose
