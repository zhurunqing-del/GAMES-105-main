...
def part1_inverse_kinematics(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    joint_orientations_local = np.zeros_like(joint_orientations)
    for i in range(len(meta_data.joint_name)):
        parent_idx =  meta_data.joint_parent[i]
        if parent_idx == -1:
            joint_orientations_local[i] = joint_orientations[parent_idx]
        else:
            joint_orientations_local[i] = (R.from_quat(joint_orientations[parent_idx]).inv() * R.from_quat(joint_orientations[i])).as_quat()
    ...
    for iter in range(max_iterations):
        ...
        for i in range(len(meta_data.joint_name)):
            if i in path2:
                ...
                if i in path1:
                    ...
                    parent_idx = meta_data.joint_parent[i]
                    orientations_local =  R.from_quat(joint_orientations_local[i])
                    parent_rot = R.from_quat(ik_joint_orientations[parent_idx])
                    ik_joint_positions[i] = ik_joint_positions[parent_idx] + parent_rot.apply(joint_offsets[i])
                    ik_joint_orientations[i] =(parent_rot * orientations_local).as_quat()
        ...
        for i, joint_idx in enumerate(movable_joints):
            ...
            for k,idx in enumerate(path2):
                if k >2:
                    parent_idx =path2[k]
                    child_idx = path2[k-1]
                    parent_orient = R.from_quat(ik_joint_orientations[parent_idx])
                    move = (ik_joint_positions[parent_idx] + parent_orient.apply(joint_offsets[child_idx]))-ik_joint_positions[child_idx]
                    ik_joint_positions[parent_idx] -=move
...
def part2_inverse_kinematics(meta_data, joint_positions, joint_orientations, relative_x, relative_z, target_height):
    ...
    # 预计算T-pose下的局部偏移
    joint_offsets = np.zeros((len(meta_data.joint_name), 3))
    for i in range(len(meta_data.joint_name)):
        parent_idx =  meta_data.joint_parent[i]
        if parent_idx == -1:
            joint_offsets[i] = np.zeros(3)
    ...
    # joint_initial_rotations = np.zeros((len(meta_data.joint_name), 4))
    # for i in range(len(meta_data.joint_name)):
    #     parent_idx = meta_data.joint_parent[i]
    #     if parent_idx == -1:
    #         joint_initial_rotations[i] = np.array([0, 0, 0, 1])
    #     else:
    #         offset = joint_offsets[i]
    #         offset /= np.linalg.norm(offset)
    #         ref = np.array([0, 1, 0])
    #         rot, _ = R.align_vectors([offset], [ref])
    #         joint_initial_rotations[i] = rot.as_quat()
    ...
    for iter in range(max_iterations):
        ...
        #     parent_idx = meta_data.joint_parent[idx]
        #     parent_rot = R.from_quat(ik_joint_orientations[parent_idx])
        #     ik_joint_positions[idx] = ik_joint_positions[parent_idx] + parent_rot.apply(joint_offsets[idx])
        # ik_joint_positions[path_to_end_effector[-1]]

        # # 2. 计算误差
        # target_pose = [relative_x,target_height , relative_z] +ik_joint_positions[0]
        # end_effector_pos = ik_joint_positions[path_to_end_effector[-1]]
        # error =    end_effector_pos-target_pose
        ...
        # # 3. 构建雅可比矩阵 J
        # # 路径中除了末端执行器外的所有关节都可以旋转
        # movable_joints = path_to_end_effector
        # J = np.zeros((3, len(movable_joints) * 3))
        
        # for i, joint_idx in enumerate(movable_joints):
        #     joint_pos = ik_joint_positions[0]+meta_data.joint_initial_position[joint_idx]
        #     joint_orientation = R.from_quat(ik_joint_orientations[joint_idx])
        ...
        #     # 转换为旋转对象并更新
        #     delta_rot = R.from_rotvec(d_theta)
        #     current_rot = R.from_quat(joint_worldrotation[joint_idx])
        #     new_rot =  current_rot
